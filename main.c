...
void parser_eat(Parser *parser, TokenType type) {
    printf("Eating token: %s \t| Type: %d\n", parser->current_token.value, parser->current_token.type); // FIXME: Remove
...
// Parse filename
void filename(Parser *parser) {
    parser_eat(parser, TOKEN_FILENAME);
}
...
// Main function
int main() {
    char input[] = "ls -l | grep '*.txt' > output.txt & echo hello >@ 192.168.1.10:8080";
    printf("Input: %s\n", input);
